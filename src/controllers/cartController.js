...
const createOrder = (req, res) => {
  ...
  try {
    ...
    const insertCartQuery = "INSERT INTO donhang (idUser, trangThai, ngayDatHang) VALUES (?, 'unpaid', NOW())";
...
const addToCart = (req, res) => {
  ...
  try {
    if (!idDonHang || !idSanPham || !soLuong) {
      throw new Error("Missing required fields: idDonHang, idSanPham, soLuong");
    }

    // Insert detail cart record
    const insertDetailCartQuery = "INSERT INTO chitietdonhang (idDonHang, idSanPham, soLuong) VALUES (?, ?, ?)";
...
const updateDetailCartTotal = (cart_id, idSanPham, callback) => {
  const query = `
...
const clearCart = (req, res) => {
  ...
  const deleteCartQuery = "DELETE FROM donhang WHERE idUser = ? AND trangThai = 'unpaid'";
